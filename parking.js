...
function playAudio(audio) {
  if (audio) {
    audio.currentTime = 1.5; // Rewind to start
...
function gameLoop() {
  ...
  if (engine === "frame-based") {
    ...
    if (!lose && !win) {
      ...
      if (rotating && moving) {
        turnSpeedacc = Math.min(
          turnSpeedacc + 0.0008 * (8 * timepassed),
          maxTurnAcceleration,
        );
      } else {
        turnSpeedacc = 0;
...
function borderCheck() {
  ...
  if (
    centerX < safeDistance ||
    centerX > canvas.width - safeDistance ||
    centerY < safeDistance ||
    centerY > canvas.height - safeDistance
  ) {
...
function drawLevel3() {
  ...
  ctx.drawImage(
    parkingSpotImg,
    parkingSpotX - parkingSpotWidth * 2,
    parkingSpotY,
    parkingSpotWidth,
    parkingSpotHeight,
  );
  ctx.drawImage(
    carImg,
    parkingSpotX - parkingSpotWidth * 2 + 25,
    parkingSpotY + 20,
    carWidth,
    carHeight,
  );
...
function drawLevel4() {
  ...
  let parkingSpotX = canvas.width / 2 - 120;
  let parkingSpotY = canvas.height / 2 - 100;
  let spotWidth = 90;
  let spotHeight = 110;
  ...
  let leftCarX = parkingSpotX - carBlockW - 10;
  let leftCarY = parkingSpotY - 30;

  let rightCarX = parkingSpotX + spotWidth + 10;
  let rightCarY = parkingSpotY - 30;
  ...
  ctx.drawImage(carImg, leftCarX, leftCarY, carBlockW, carBlockH);
  checkObstacleCollision(leftCarX, leftCarY, carBlockW, carBlockH);

  ctx.drawImage(carImg, rightCarX, rightCarY, carBlockW, carBlockH);
  checkObstacleCollision(rightCarX, rightCarY, carBlockW, carBlockH);

  if (!start) {
    carX = canvas.width - 100;
  ...
  const inside =
    centerX > parkingSpotX &&
    centerX < parkingSpotX + spotWidth &&
    centerY > parkingSpotY &&
    centerY < parkingSpotY + spotHeight;

  const isStraight = Math.abs(Math.sin(angle)) < 0.15;

  if (inside && isStraight && !moving) {
    setTimeout(() => {
      win = true;
    }, 500);
  }

  if (borderCollision && !lose) {
    playAudio(crashSound);
    ctx.drawImage(crash, carX, carY, 70, 70);
    lose = true;
  }
